package paint.Shapes;

import java.awt.*;
import java.util.*;

public class Polylinha extends Drawable
{
    private final ArrayList<Point> pontos;

    public Polylinha(boolean isFilled, boolean isGradient, boolean isDashedLine, int lineWidth, int dashLength, Color primaryColor, Color secondaryColor) {
        super(isFilled, isGradient, isDashedLine, lineWidth, dashLength, primaryColor, secondaryColor);
        this.pontos = new ArrayList();
    }
    
    @Override
    public void draw(Graphics g) {
        Graphics2D gg = (Graphics2D) g;
        
        if(this.isIsGradient())
        {
            gg.setPaint(new GradientPaint(new Point(0,0),this.getPrimaryColor(),new Point(10,10),this.getSecondaryColor(),true));
        }
        else
        {
            gg.setPaint(this.getPrimaryColor());
        }
        
        if(this.isIsDashedLine())
        {
            float[] dashes = {this.getDashLength()};
            gg.setStroke(new BasicStroke(this.getLineWidth(),BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND,0,dashes,dashes[0]));
        }
        else
        {
            gg.setStroke(new BasicStroke(this.getLineWidth()));
        }
        
        int[] xPoints = new int[this.pontos.size()];
        int[] yPoints = new int[this.pontos.size()];
        int cont = 0;
        
        for(Point p : this.pontos)
        {
            xPoints[cont] = (int)p.getX();
            yPoints[cont] = (int)p.getY();
            cont++;
        }
        
        gg.drawPolyline(xPoints, yPoints, cont);
    }

    @Override
    public void clicar(Point ponto) {
        this.pontos.add(ponto);
        this.pontos.add(ponto);
    }

    @Override
    public void arrastar(Point ponto) {
        
    }

    @Override
    public void moverMouse(Point ponto) {
        this.pontos.set(this.pontos.size()-1, ponto);
    }

    @Override
    public void cliqueDireito(Point ponto) {
        this.pontos.remove(this.pontos.size()-1);
        this.finish();
    }

    @Override
    public void soltarMouse(Point ponto) {
        
    }
}
